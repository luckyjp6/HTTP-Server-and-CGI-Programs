...
{
public:
    my_client(tcp::socket shell_, tcp::socket web_) 
    : shell(std::move(shell_)), web(std::move(web_))
    {
    }
    ...
    void start(int index)
    {
		...
		if (!in) cout << "successfully open " << my_query[id].file.data() << endl;
		else cout << "can't open file " << my_query[id].file.data() << endl;
...
private:
    ...
    void do_read_shell()
    {
        ...
        [this, self](boost::system::error_code ec, std::size_t length)
        {
			...
			{
				cout << "failed to read from shell " << id << endl;
				close_client();
			}
        });
	}
	...
	void do_write(string msg)
  {
	...
	void do_console()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			...
			make_shared<my_client> (std::move(shell), std::move(web))->start(i);
